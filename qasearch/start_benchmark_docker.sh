...
cd /code/the-pattern-api/qasearch && python3 export_load_bert.py
gears-cli run --host rgcluster --port 30001 tokeniser_gears_redisai.py --requirements requirements.txt
gears-cli run --host rgcluster --port 30001 qa_redisai_keymiss_no_cache_np.py
...
# redis-cli -c -p 30001 -h rgcluster get "bertqa{356}_PMC140314.xml:{356}:47_When air samples collected"
#redis-cli -c -p 30001 -h rgcluster get "bertqa{356}_PMC140314.xml:{356}:8_Who performs viral transmission among adults" 
# redis-cli -c -p 30003 -h rgcluster get "bertqa{8YG}_PMC302072.xml:{8YG}:10_Who performs viral transmission among adults"
# then run benchmark via docker: docker exec -it container_name  /bin/sh -c "redis-benchmark -r 10000 -n 10000 PING"
# or via command line:
# redis-benchmark -p 30003 -h rgcluster --precision 3 --csv -n 100 get "bertqa{8YG}_PMC302072.xml:{8YG}:10_Who performs viral transmission among adults"
# or without csv and precision
# redis-benchmark -p 30003 -h rgcluster -n 10 get "bertqa{8YG}_PMC302072.xml:{8YG}:10_Who performs viral transmission among adults"
